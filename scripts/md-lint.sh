...
#
# Version History:
# + v1.2 - Switch to the markdownlint-cli2 command and the .markdownlint-cli2.jsonc config.
...
DEFAULT_LINT_CONFIG=".markdownlint-cli2.jsonc"
LINT_CONFIG=""
...
function check_markdownlint_installed {
  if ! command -v $MARKDOWNLINT_CMD &> /dev/null; then
    ...
    printf "pnpm install -g markdownlint-cli2\nor\nbrew install markdownlint-cli2\n"
    exit 1
  fi
}
...
function run_markdownlint {
  ...
  if [ -z "$lint_config" ]; then
    ...
    $MARKDOWNLINT_CMD --config "$lint_config" "$file_path" >> $LOG_FILE 2>&1
  fi
